...
using Brueder.Architecture.Base.Entityes;
using Brueder.Architecture.Base.Extensions;
...
public sealed class Repository<TEntity> : IRepository<TEntity> where TEntity : class
{
    ...
    {
        _dbContext = dbContext ?? throw new ArgumentNullException($"{nameof(dbContext)} is not exist");
